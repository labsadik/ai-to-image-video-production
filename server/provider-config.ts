...
export async function resolveProviderConfig(providerId: string, modelId: string): Promise<RuntimeProviderConfig> {
  ...
  if (!['google_gemini', 'fal_video', 'pixazo_image', 'pixazo_video', 'groq_vision'].includes(String(provider.protocol))) throw new Error(`Unsupported AI provider protocol: ${provider.protocol}`);
  return {
