...
export const DISPLAY_PLANS: readonly DisplayPlan[] = [
  {
    id: 'free',
    name: 'Free',
    description: 'Explore the studio and learn the workflow.',
    monthlyCredits: 10,
    maxUploadsPerProject: 1,
    watermark: true,
    monthlyFallbackMinor: 0,
    features: ['10 credits / month', '1 upload / project', 'Visible watermark', 'Platform presets', 'Private project storage'],
    icon: Gauge,
    premium: false,
  },
  {
    id: 'pro',
    name: 'Pro',
    description: 'For independent creators who need more control.',
    monthlyCredits: 100,
    maxUploadsPerProject: 10,
    watermark: false,
    monthlyFallbackMinor: 1000,
    features: ['100 credits / month', '10 uploads / project', 'No visible watermark', 'Premium generation', 'Signed provenance'],
    icon: Sparkles,
    premium: true,
  },
  {
    id: 'business',
    name: 'Business',
    description: 'Higher capacity for teams and production work.',
    monthlyCredits: 1000,
    maxUploadsPerProject: 30,
    watermark: false,
    monthlyFallbackMinor: 3000,
    features: ['1,000 credits / month', '30 uploads / project', 'No visible watermark', 'Higher throughput', 'Priority workspace controls'],
    icon: Crown,
    premium: true,
  },
];
...
export const PLAN_COMPARISON_FEATURES = [
  ...
  { label: 'Premium generation', key: 'premium', format: (plan: DisplayPlan) => plan.id === 'free' ? 'Not included' : 'Included' },
...
export function qualifiesForUpcomingFeatures(planId: PlanId, period: BillingPeriod) {
  return planId !== 'free' && period >= 6;
}

export const planHighlights = [
  { icon: Upload, label: 'Controlled uploads' },
  { icon: ShieldCheck, label: 'Safety + private storage' },
];
