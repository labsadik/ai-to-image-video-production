...
export async function POST(request: Request) {
  try {
    ...
    if (!body.mimeType || !ALLOWED_TYPES.has(body.mimeType) || !body.byteSize || body.byteSize <= 0 || body.byteSize > MAX_BYTES) {
    ...
    if (body.projectId) {
      ...
      const { count } = await admin.from('assets').select('id', { count: 'exact', head: true }).eq('project_id', body.projectId);
    ...
    const { data: asset, error: assetError } = await admin.from('assets').insert({
      ...
      status: 'uploading',
      metadata: { upload_issued_at: new Date().toISOString() },
    }).select('id').single();
    if (assetError || !asset) return NextResponse.json({ error: assetError?.message ?? 'Unable to create asset record' }, { status: 500 });
