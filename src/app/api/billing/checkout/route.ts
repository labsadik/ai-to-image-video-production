...
function isBillingPeriod(value: unknown): value is BillingPeriod {
  return typeof value === 'number' && BILLING_PERIODS.includes(value as BillingPeriod);
}
...
async function createStripeCheckout(params: Record<string, string>) {
...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { kind?: 'plan' | 'addon'; planId?: unknown; period?: unknown };
    ...
    if (kind === 'addon') {
      ...
      const params: Record<string, string> = {
        ...
        success_url: `${base}/dashboard/billing?checkout=success&kind=addon`,
    ...
    const params: Record<string, string> = {
      mode: 'subscription',
      'line_items[0][quantity]': '1',
      success_url: `${base}/dashboard/billing?checkout=success&kind=plan`,
      cancel_url: `${base}/dashboard/billing?checkout=cancelled`,
      client_reference_id: user.id,
      customer_creation: 'always',
      billing_address_collection: 'required',
      'metadata[user_id]': user.id,
      'metadata[purchase_kind]': 'plan',
      'metadata[plan_id]': body.planId,
      'metadata[country_code]': country,
      'subscription_data[metadata][user_id]': user.id,
      'subscription_data[metadata][plan_id]': body.planId,
      'subscription_data[metadata][country_code]': country,
    };
    if (email) params.customer_email = email;
    ...
    if (body.period === 1 && price.stripe_price_id) {
      ...
      params['line_items[0][price_data][product_data][description]'] = `${body.planId === 'pro' ? 50 : 100} monthly Solamentis credits and your plan workspace limits.`;
