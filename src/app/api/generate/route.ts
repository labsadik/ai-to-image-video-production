...
import type { GenerationQuality, Operation, FeatureCategory } from '@/core/ai';
...
export async function POST(request: Request) {
  try {
    ...
    if(!prompt||prompt.length>8000||!quality||!operations.has(operation)||!categories.has(category)) return NextResponse.json({error:'Invalid generation request'},{status:400});
    ...
    const job=await createGenerationJob({userId:user.id,projectId,plan:planId as keyof typeof GENERATION_PLANS,operation:operation as Exclude<Operation,'detectImage'>,category,prompt,size:platform??`${spec.width}x${spec.height}`,width:spec.width,height:spec.height,quality:quality as GenerationQuality,platform,referenceImageStoragePaths,idempotencyKey:request.headers.get('Idempotency-Key')??undefined});
