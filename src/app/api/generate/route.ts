...
export async function POST(request: Request) {
  try {
    ...
    const referenceImageStoragePaths=Array.isArray(body.referenceImageStoragePaths)?body.referenceImageStoragePaths.filter((value):value is string=>typeof value==='string'&&value.length>0).slice(0,1):[];
    if(!prompt||prompt.length>8000||!qualities.has(quality)||!operations.has(operation))return NextResponse.json({error:'Invalid generation request'},{status:400});
    ...
    if(quality==='premium'&&GENERATION_PLANS[profile.plan as keyof typeof GENERATION_PLANS].credits.premium<=0)return NextResponse.json({error:'Premium generation is not available on this plan'},{status:403});
    const spec=platform?PLATFORM_SPECS[platform]:{width:1024,height:1024,maxBytes:8_000_000,mimeTypes:['image/webp'] as const};
    const job=await createGenerationJob({userId:user.id,projectId,plan:profile.plan as keyof typeof GENERATION_PLANS,operation:operation as Exclude<Operation,'detectImage'>,prompt,size:platform??`${spec.width}x${spec.height}`,width:spec.width,height:spec.height,quality,platform,referenceImageStoragePaths,idempotencyKey:request.headers.get('Idempotency-Key')??undefined});
