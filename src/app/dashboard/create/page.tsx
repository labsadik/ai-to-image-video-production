  ...
  return <div className="space-y-8">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-500">One authoritative image is stored for each generation. History, Studio, preview, editing, and download all use that same file.</p>
      </div>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_520px]">
      <form onSubmit={submit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-5">
          ...
          <div><p className="text-sm font-semibold">{historyId ? 'Saved generation' : 'New generation'}</p><p className="text-xs text-slate-400">Provider and model selection remain configuration-driven.</p></div>
        </div>
    ...
    {modalOpen && imageUrl && <div className="fixed inset-0 z-[100] bg-slate-950/95 p-4 sm:p-8" onWheel={onZoomWheel}>
      <div className="flex h-full flex-col overflow-hidden rounded-3xl border border-white/10 bg-slate-900 shadow-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-slate-950 p-4 sm:p-8"><div className="flex min-h-full min-w-full items-center justify-center"><img src={imageUrl} alt="Expanded generated visual" style={{ width: `${zoom * 100}%`, maxWidth: zoom <= 1 ? '100%' : 'none' }} className="h-auto object-contain transition-[width] duration-150" /></div></div>
        <div className="flex shrink-0 items-center justify-between border-t border-white/10 px-4 py-3 text-xs text-slate-400 sm:px-5"><span>{output?.width ?? spec.width} × {output?.height ?? spec.height}</span><button type="button" disabled={downloading} onClick={() => void downloadCurrent()} className="inline-flex items-center gap-2 rounded-xl bg-white px-3.5 py-2 text-xs font-semibold text-slate-900 disabled:opacity-50">{downloading ? <Loader2 className="size-4 animate-spin" /> : <Download className="size-4" />} Download</button></div>
      </div>
    </div>}
  </div>;
}
