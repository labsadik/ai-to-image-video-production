...
const examples = {
  image: {
    bad: 'A nice car',
    good: 'Luxury black sports car outside a glass villa at sunset, cinematic lighting, realistic reflections, premium automotive advertising photography, 16:9.',
    super: 'Subject: luxury black sports car. Environment: modern glass villa at golden hour. Lighting: warm sunset rim light plus soft studio fill. Camera: low 35mm automotive commercial look. Composition: hero product centered with negative space for headline. Materials: glossy paint, realistic reflections. Mood: premium, elegant, high-end. Constraints: no people, no logos, clean background.'
  },
  video: {
    bad: 'Make an ad for shoes',
    good: 'Create a 8-second premium running-shoe advertisement. Show the shoe rotating slowly above a wet reflective studio floor, dramatic soft lighting, close product details, clean background, cinematic commercial motion, no people, no text, no audio.',
    super: 'Product: premium white running shoe. Duration: 8 seconds. Scene: dark luxury studio with reflective wet floor. Action: shoe enters frame, rotates 180 degrees, camera pushes in, final hero angle holds for 1 second. Camera: controlled commercial dolly plus macro detail. Lighting: soft key, subtle rim, polished reflections. Motion: smooth and physically plausible. Mood: premium athletic performance. Constraints: no people, no text, no logos, no audio.'
  },
  analysis: {
    bad: 'Is this AI?',
    good: 'Analyze this image for likely AI generation, digital manipulation, compositing, camera-original characteristics, metadata/provenance clues, compression artifacts, and visual inconsistencies. Return confidence and evidence.',
    super: 'Perform a deep forensic authenticity analysis. Estimate AI-generation likelihood, editing/manipulation likelihood, camera-original likelihood, and confidence. Examine texture/frequency anomalies, local detail consistency, resampling/compression clues, metadata when available, compositing indicators, and signs associated with AI image pipelines. Return evidence, uncertainty, and limitations. Do not claim certainty.'
  }
} as const;
...
export default function MediaStudioPage() {
  ...
  async function submitImage() {
    ...
    try {
      const response = await fetch('/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ operation: 'generateImage', prompt: prompt.trim(), platform: 'ad_creative', quality: quality === 'premium' ? 'premium' : quality === 'standard' ? 'standard' : 'preview', width: 1024, height: 1024, size: '1024x1024', referenceImageStoragePaths: reference?.path ? [reference.path] : [] }) });
      const data = await response.json() as { jobId?: string; error?: string };
      if (!response.ok || !data.jobId) throw new Error(data.error || 'Image generation could not be started.');
      const poll = async () => {
        const status = await fetch(`/api/generate/${data.jobId}`, { cache: 'no-store' }).then(r => r.json()) as { job?: { status: string; error_message?: string | null }; output?: { url?: string | null } | null; error?: string };
        if (status.job?.status === 'succeeded' && status.output?.url) { setResult(status.output.url); setMessage('Image ready.'); setLoading(false); return; }
        if (status.job?.status === 'failed' || status.job?.status === 'cancelled') { setMessage(status.job.error_message || 'Image generation failed.'); setLoading(false); return; }
  ...
  async function submitAnalysis() {
    if (!reference?.file) { setMessage('Attach an image to analyze.'); return; }
    setLoading(true); setMessage(''); setAnalysis(null);
    try {
      const form = new FormData();
      form.append('image', reference.file);
      form.append('level', quality as AnalysisLevel);
  ...
  async function submitVideo() {
    ...
    try {
      ...
      setMessage(`Video job queued · ${data.model ?? 'OpenRouter test model'}`);
      const poll = async () => {
        const status = await fetch(`/api/video-ad/${data.jobId}`, { cache: 'no-store' }).then(r => r.json()) as { job?: { status: string; error_message?: string | null }; output?: { url?: string | null } | null; error?: string };
        if (status.job?.status === 'succeeded' && status.output?.url) { setResult(status.output.url); setMessage('Video ready.'); setLoading(false); return; }
        if (status.job?.status === 'failed' || status.job?.status === 'cancelled') { setMessage(status.job.error_message || 'Video generation failed.'); setLoading(false); return; }
  ...
  return <div className="space-y-6 sm:space-y-8">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(360px,520px)]">
      <Card><CardHeader><div className="flex items-center gap-3">{tab==='image'?<Sparkles className="size-5"/>:tab==='analysis'?<Camera className="size-5"/>:<Video className="size-5"/>}<div><p className="text-sm font-semibold">{tab==='image'?'Image generation':tab==='analysis'?'Image authenticity analysis':'Silent video ad generation'}</p><p className="text-xs text-slate-400">Credits, plan restrictions, and provider routing remain centralized.</p></div></div></CardHeader><CardBody className="space-y-5">
        {tab==='analysis' ? <ImageUpload current={reference} onChange={setReference}/> : null}
        {tab==='image' ? <ImageUpload current={reference} onChange={setReference}/> : null}
        {tab==='analysis' ? <label className="block text-sm font-medium">Analysis depth<select value={quality} onChange={e=>setQuality(e.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3 py-3"><option value="basic">Basic · 2 credits</option><option value="medium">Medium · 5 credits</option><option value="hard">Hard · 12 credits</option></select></label> : null}
        {tab==='image' ? <label className="block text-sm font-medium">Quality<select value={quality} onChange={e=>setQuality(e.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3 py-3"><option value="preview">Preview · existing plan pricing</option><option value="standard">Standard · existing plan pricing</option><option value="premium">Premium · existing plan pricing</option></select></label> : null}
        {tab==='video' ? <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium">Duration<select value={duration} onChange={e=>setDuration(Number(e.target.value))} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3 py-3">{[3,4,5,6,8,10,12,15,20,30].map(v=><option key={v} value={v}>{v} seconds</option>)}</select></label><label className="block text-sm font-medium">Quality<select value={videoQuality} onChange={e=>setVideoQuality(e.target.value as VideoQuality)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3 py-3"><option value="standard">Standard Ad · 9 credits</option><option value="high_end">High-End Ad · 25 credits</option></select></label></div> : null}
        {tab!=='analysis' ? <label className="block text-sm font-medium">Prompt<textarea required minLength={3} rows={9} value={prompt} onChange={e=>setPrompt(e.target.value)} className="mt-2 min-h-40 w-full resize-y rounded-2xl border border-slate-200 px-4 py-3.5 leading-6" placeholder={tab==='video'?'Describe product, motion, camera, scene, lighting, timing, and constraints.':'Describe subject, composition, lighting, style, and constraints.'}/></label> : null}
