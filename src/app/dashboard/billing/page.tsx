...
export default async function BillingPage(){
  ...
  return <div className="space-y-7 sm:space-y-8"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-slate-400">Billing</p><h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-4xl">Plans & subscription</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Compare every plan, choose one month, six months, or one year, and review the exact regional pricing before checkout.</p></div><div className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600 shadow-sm"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-slate-100"><CreditCard className="size-4"/></span><span>Billing country: <strong className="font-semibold text-slate-900">{country}</strong> · Currency: <strong className="font-semibold text-slate-900">{currency}</strong></span></div><PlanComparison prices={priceMap} currency={currency} initialPlan={(profile?.plan_id==='business'||profile?.plan_id==='free'||profile?.plan_id==='pro')?profile.plan_id:'pro'}/></div>;
}
