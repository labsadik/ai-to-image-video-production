...
function formatDate(value: string | null | undefined) { if (!value) return null; return new Intl.DateTimeFormat(undefined, { dateStyle: 'medium' }).format(new Date(value)); }

export default async function BillingPage(){
  const client=await getSupabaseServerClient(); const {data:{user}}=await client.auth.getUser(); if(!user)return null;
  const admin=getSupabaseAdmin();
  const [{data:profile},{data:prices},{data:packs},{data:regions},{data:nextGrant},{data:subscription},{data:transactions}]=await Promise.all([
    ...
    admin.from('billing_transactions').select('id,kind,status,plan_id,description,amount_minor,currency,purchased_at,period_end,receipt_url').eq('user_id',user.id).order('purchased_at',{ascending:false}).limit(50),
  ]);
  ...
  const priceMap=Object.fromEntries(regional.map((price)=>[price.plan_id,Number(price.unit_amount_minor)]));
  ...
  return <div className="space-y-7 sm:space-y-8">
    <div><p className="text-xs font-semibold uppercase tracking-[.18em] text-slate-400">Billing</p><h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-4xl">Plans & credit wallet</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Manage your subscription, expiry dates, credit purchases, and verified payment history from one place.</p></div>
    ...
    <BillingHistory transactions={(transactions??[]) as BillingTransaction[]} />
  </div>;
}
