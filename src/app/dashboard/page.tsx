...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: projects }, { data: jobs }] = await Promise.all([
    admin.from('profiles').select('full_name,plan_id,credits,credits_reserved,monthly_credits,addon_credits,country_name').eq('id', user.id).single(),
  ...
  return <div className="space-y-7 sm:space-y-8">
    ...
    <CreditWallet monthly={monthly} addon={addon} />
