...
function normalizeSafetyStage(stage: string): SafetyEventStage {
  switch (stage) {
    ...
    case 'upload':
    case 'upload_image_moderation':
    case 'upload_validation': return 'upload';
    case 'generation':
    case 'post_generation_image_moderation': return 'generation';
    case 'edit':
    case 'edit_image_moderation': return 'edit';
...
export async function recordSafetyEvent(input: { userId?: string | null; jobId?: string | null; assetId?: string | null; stage: string; decision: SafetyDecision; reasons: string[]; score?: number; policyVersion?: number; providerId?: string | null; modelKey?: string | null }) {
  const admin = getSupabaseAdmin();
