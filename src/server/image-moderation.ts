...
async function moderateWithGoogle(input: {
  ...
  const instruction = [
    ...
    'score must be a 0..1 risk/confidence score.',
  ].join('\n');
...
async function moderateWithGenericJson(input: {
  ...
  if (cfg.auth === 'api-key') headers['x-api-key'] = input.apiKey;
  else if (cfg.auth === 'custom') headers[String(cfg.authHeader ?? 'authorization')] = input.apiKey;
  else if (cfg.auth !== 'none') headers.authorization = `Bearer ${input.apiKey}`;
...
export async function moderateImage(input: {
  ...
  const result = config.protocol === 'google_gemini'
    ...
    : await moderateWithGenericJson({
        ...
        base64: input.base64,
        requestConfig: config.requestConfig as unknown as Record<string, unknown>,
      });
