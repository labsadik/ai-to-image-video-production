...
function parseModelJson(text: string): { decision?: unknown; reasons?: unknown; score?: unknown } {
  const cleaned = text.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, '');
  try {
    return JSON.parse(cleaned) as { decision?: unknown; reasons?: unknown; score?: unknown };
  } catch {
...
async function moderateWithGoogle(input: {
  apiKey: string;
  model: string;
  mimeType: string;
  base64: string;
  ...
  const block = input.rules.blockPatterns ?? [];
  const review = input.rules.reviewPatterns ?? [];
  const instruction = [
    'You are Solamentis production image-safety moderation.',
    'Inspect the supplied image only. Do not generate, transform, or describe sexual content in detail.',
    'Classify the image into exactly one decision: allow, review, or block.',
    'block categories:',
    ...block.map(item => `- ${item}`),
    'review categories:',
    ...review.map(item => `- ${item}`),
    'Use block for a clearly matching blocked category. Use review when content is ambiguous, explicit/sexualized, or high-risk but not clearly blocked. Use allow only when no listed concern is present.',
    'Return JSON only: {"decision":"allow|review|block","reasons":["short category reason"],"score":0.0}.',
    'score must be a 0..1 confidence/risk score consistent with the decision.',
  ].join('\n');
  ...
  try {
    ...
    return {
      decision: 'review',
      reasons: [error instanceof Error ? `Vision moderation unavailable: ${error.message}` : 'Vision moderation unavailable'],
      provider: 'google',
      model: input.model,
    };
  }
...
function renderTemplate(value: unknown, values: Record<string, unknown>): unknown {
  if (typeof value === 'string') {
    ...
    return value.replace(/\{\{([a-zA-Z0-9_]+)\}\}/g, (_, name: string) => String(values[name] ?? ''));
  }
...
async function moderateWithGenericJson(input: {
  provider: string;
  baseUrl: string;
  timeoutMs: number;
  apiKey: string;
  ...
  const auth = cfg.auth === 'api-key' ? { 'x-api-key': input.apiKey } : cfg.auth === 'custom' ? { [String(cfg.authHeader ?? 'authorization')]: input.apiKey } : cfg.auth === 'none' ? {} : { authorization: `Bearer ${input.apiKey}` };
  const headers = { 'content-type': 'application/json', ...(cfg.headers && typeof cfg.headers === 'object' ? cfg.headers as Record<string, string> : {}), ...auth };
  const values = { model: input.model, mimeType: input.mimeType, base64: input.base64 };
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), input.timeoutMs);
  try {
    const response = await fetch(`${input.baseUrl.replace(/\/$/, '')}/${path.replace(/^\//, '')}`, {
      method: typeof cfg.method === 'string' ? cfg.method : 'POST',
      headers,
      body: JSON.stringify(renderTemplate(cfg.body ?? { model: '{{model}}', image: { mime_type: '{{mimeType}}', data: '{{base64}}' } }, values)),
    ...
    return normalizeResult({
      decision: getPath(payload, String(result.decisionPath ?? 'decision')),
      reasons: getPath(payload, String(result.reasonsPath ?? 'reasons')),
      score: getPath(payload, String(result.scorePath ?? 'score')),
...
export async function moderateImage(input: {
  ...
  const { data: policy, error: policyError } = await admin
    ...
    .select('version, rules, moderation_provider_id, moderation_model_id')
    .eq('status', 'active')
  ...
  const adapterResult = config.protocol === 'google_gemini'
  ...
  const { error: eventError } = await admin.from('safety_events').insert({
    ...
    score: adapterResult.score ?? null,
    provider_id: adapterResult.provider,
    model_key: adapterResult.model,
  });
