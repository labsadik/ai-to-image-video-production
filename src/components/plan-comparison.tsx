...
export function PlanComparison({ prices = {}, currency = 'USD', initialPlan = 'pro' }: Props) {
  ...
  const selectedPlan = useMemo(() => DISPLAY_PLANS.find((plan) => plan.id === selected), [selected]);

  function requestCheckout() {
    if (selected === initialPlan || selected === 'free') return;
    setError('');
    setConfirmOpen(true);
  }
  ...
  async function checkout() {
    ...
    try {
      ...
      const response = await fetch('/api/billing/checkout', {
        ...
        body: JSON.stringify({ kind: 'plan', planId: selected, period, requestId }),
      });
  ...
  return <>
    ...
    <div className="grid gap-5 lg:grid-cols-3">{DISPLAY_PLANS.map((plan)=>{const Icon=plan.icon;const active=selected===plan.id;const current=initialPlan===plan.id;const upcoming=qualifiesForUpcomingFeatures(plan.id,period);return <article key={plan.id} className={`relative flex min-w-0 flex-col overflow-hidden rounded-3xl border p-6 shadow-sm transition ${active?'border-slate-950 bg-slate-950 text-white shadow-xl':'border-slate-200 bg-white hover:-translate-y-0.5 hover:shadow-md'}`}>
      ...
      <div className="mt-6 flex-1 space-y-3">{plan.features.map((feature)=><div key={feature} className="flex gap-2 text-sm"><Check className={`mt-0.5 size-4 shrink-0 ${active?'text-white':'text-emerald-600'}`}/><span>{feature}</span></div>)}{upcoming&&<div className={`rounded-xl border p-3 text-xs font-semibold ${active?'border-white/10 bg-white/5 text-white':'border-amber-200 bg-amber-50 text-amber-800'}`}>Upcoming features are available only with Growth on a 6-month or 1-year term.</div>}</div>
      <button type="button" onClick={()=>setSelected(plan.id)} className={`mt-7 min-h-11 rounded-xl px-4 text-sm font-semibold ${active?'bg-white text-slate-950':'bg-slate-950 text-white hover:bg-slate-800'}`}>{current?'Current plan':active?'Selected':'Select plan'}</button>
    </article>})}</div>

    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-slate-400">Checkout</p><p className="mt-1 text-sm font-semibold text-slate-950">{selected==='free'?'Free plan requires no payment.':selected===initialPlan?`You're already on the ${selectedPlan?.name ?? selected} plan.`:`Continue with ${selectedPlan?.name ?? selected} · ${periodLabel(period)}`}</p><p className="mt-1 text-xs text-slate-500">You will review the selected plan first, then continue to Stripe Checkout for the real payment.</p></div><button type="button" onClick={requestCheckout} disabled={loading||selected===initialPlan||selected==='free'} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-45">{loading&&<Loader2 className="size-4 animate-spin" />}{loading?'Opening checkout…':selected===initialPlan?'Current plan':'Confirm & continue to payment'}</button></div></section>

    {confirmOpen&&selectedPlan&&<div className="fixed inset-0 z-[130] grid place-items-center bg-slate-950/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Confirm plan purchase" onClick={()=>!loading&&setConfirmOpen(false)}><div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl" onClick={(event)=>event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-slate-400">Confirm purchase</p><h2 className="mt-2 text-xl font-semibold text-slate-950">{selectedPlan.name} · {periodLabel(period)}</h2></div><button type="button" disabled={loading} onClick={()=>setConfirmOpen(false)} aria-label="Close confirmation" className="rounded-xl p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-40"><X className="size-5"/></button></div><div className="mt-6 space-y-3 rounded-2xl bg-slate-50 p-4"><div className="flex justify-between gap-4 text-sm"><span className="text-slate-500">Billing country</span><strong className="text-slate-900">Regional pricing</strong></div><div className="flex justify-between gap-4 text-sm"><span className="text-slate-500">Term</span><strong className="text-slate-900">{periodLabel(period)}</strong></div><div className="flex justify-between gap-4 text-sm"><span className="text-slate-500">Total</span><strong className="text-slate-950">{currency} {(total(selected)/100).toLocaleString()}</strong></div>{qualifiesForUpcomingFeatures(selected,period)&&<div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-800">Upcoming features are included because this is Growth on a 6-month/1-year term.</div>}</div><p className="mt-4 text-xs leading-5 text-slate-500">The final amount is revalidated on the server from Supabase pricing. Payment is processed by Stripe. The plan becomes active only after the verified Stripe webhook is accepted.</p><div className="mt-6 flex gap-3"><button type="button" disabled={loading} onClick={()=>setConfirmOpen(false)} className="min-h-11 flex-1 rounded-xl border border-slate-200 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancel</button><button type="button" disabled={loading} onClick={()=>void checkout()} className="min-h-11 flex-1 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50">{loading?'Opening Stripe…':'Confirm & pay'}</button></div></div></div>}
