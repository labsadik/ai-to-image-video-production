...
export function MediaUsagePanel({ className = '' }: Props) {
  ...
  return <>
    <button type="button" onClick={openUsage} className={`inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-950 ${className}`} aria-haspopup="dialog" aria-expanded={open}>
      <Calculator className="size-4" />Usage{usage && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-700">{usage.credits.available}</span>}
    </button>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="mx-auto w-full max-w-[1500px] space-y-6 px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
            ...
            {!usage && loading ? <div className="space-y-5"><div className="h-32 animate-pulse rounded-3xl bg-white" /><div className="grid gap-5 lg:grid-cols-3"><div className="h-96 animate-pulse rounded-3xl bg-white" /><div className="h-96 animate-pulse rounded-3xl bg-white" /><div className="h-96 animate-pulse rounded-3xl bg-white" /></div></div> : usage && <>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><SummaryCard dark label="Available now" value={usage.credits.available} detail={`of ${usage.credits.monthly} monthly credits`} /><SummaryCard label="Used" value={usage.credits.used} detail={`${usage.credits.monthly > 0 ? Math.min(100, Math.round((usage.credits.used / usage.credits.monthly) * 100)) : 0}% of monthly allowance`} /><SummaryCard label="Reserved" value={usage.credits.reserved} detail="Temporarily held by active jobs" /><SummaryCard label="Uploads" value={usage.uploads.total} detail={`${usage.uploads.maxPerProject} max per project`} /></div>
              <div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><div className="flex items-center justify-between text-xs font-semibold text-slate-500"><span>Monthly credit usage</span><span>{usage.credits.used} / {usage.credits.monthly}</span></div><div className="mt-3 h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-950 transition-all" style={{ width: `${usage.credits.monthly > 0 ? Math.min(100, Math.round((usage.credits.used / usage.credits.monthly) * 100)) : 0}%` }} /></div><p className="mt-3 text-xs leading-5 text-slate-500">Available means credits you can spend right now. Reserved credits are already held by active jobs and are not counted as available.</p></div>
              <div className="grid gap-5 lg:grid-cols-3">
                <FeatureCard icon={<ImageIcon className="size-5" />} title="Image generation" subtitle="Your cost and how many more runs you can make now.">{([['Preview', usage.features.imageGeneration.preview], ['Standard', usage.features.imageGeneration.standard], ['Premium', usage.features.imageGeneration.premium]] as const).map(([label, cost]) => <CostRow key={label} label={label} cost={cost} available={usage.credits.available} maxCost={maxCost} />)}</FeatureCard>
                <FeatureCard icon={<Calculator className="size-5" />} title="Image authenticity analysis" subtitle="Basic, medium and hard analysis levels use different credit amounts.">{([['Basic', usage.features.imageAnalysis.basic], ['Medium', usage.features.imageAnalysis.medium], ['Hard', usage.features.imageAnalysis.hard]] as const).map(([label, cost]) => <CostRow key={label} label={label} cost={cost} available={usage.credits.available} maxCost={maxCost} />)}</FeatureCard>
                <FeatureCard icon={<Video className="size-5" />} title="Video generation" subtitle={`${usage.video.minDurationSeconds}–${usage.video.maxDurationSeconds}s · ${usage.video.audio ? 'audio on' : 'silent output'}`}>{([['Standard · 720p', usage.features.videoAd.standard], ['High-End · 1080p', usage.features.videoAd.high_end]] as const).map(([label, cost]) => <CostRow key={label} label={label} cost={cost} available={usage.credits.available} maxCost={maxCost} />)}</FeatureCard>
              </div>
              <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-slate-100 text-slate-700"><UploadCloud className="size-5" /></span><div><p className="text-sm font-semibold text-slate-950">Upload limits</p><p className="text-xs text-slate-500">Live count from your workspace</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Active uploads</p><p className="mt-1 text-2xl font-semibold text-slate-950">{usage.uploads.total}</p><p className="mt-1 text-xs text-slate-500">Across this workspace</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Per project</p><p className="mt-1 text-2xl font-semibold text-slate-950">{usage.uploads.maxPerProject}</p><p className="mt-1 text-xs text-slate-500">Maximum allowed</p></div></div><p className="mt-4 text-sm leading-6 text-slate-600">{usage.uploads.note}</p></div><div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><CheckCircle2 className="size-5" /></span><div><p className="text-sm font-semibold text-slate-950">How the calculator works</p><p className="text-xs text-slate-500">Simple, live math before you run a feature</p></div></div><div className="mt-5 space-y-3 text-sm leading-6 text-slate-600"><p><strong className="text-slate-950">Cost</strong> = the credits removed when that feature completes successfully.</p><p><strong className="text-slate-950">More now</strong> = floor(available credits ÷ feature cost).</p><p><strong className="text-slate-950">After 1 run</strong> = available credits − feature cost.</p></div></div></div>
            </>}
          </div>
        </div>
