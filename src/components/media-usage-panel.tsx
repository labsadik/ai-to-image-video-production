...
type Usage = {
  ...
  video: { minDurationSeconds: number; maxDurationSeconds: number; audio: boolean; quality: string; aspectRatios: string[]; aspectRatioAffectsPrice: boolean };
};
      ...
      <section className="flex h-[100dvh] w-full flex-col overflow-hidden bg-slate-50" role="dialog" aria-modal="true" aria-labelledby="media-usage-title">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto"><div className="mx-auto w-full max-w-[1500px] space-y-5 px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
          ...
          {usage && <>
            ...
            <div className="grid gap-5 lg:grid-cols-2"><FeatureCard icon={<ImageIcon className="size-5" />} title="Image authenticity analysis" subtitle="The analysis type directly changes the credit charge.">{([['Basic', usage.features.imageAnalysis.basic], ['Medium', usage.features.imageAnalysis.medium], ['Hard', usage.features.imageAnalysis.hard]] as const).map(([label, cost]) => <CostRow key={label} label={label} cost={cost} available={usage.credits.available} />)}</FeatureCard><FeatureCard icon={<Video className="size-5" />} title="Generate a silent video" subtitle={`${usage.video.minDurationSeconds}–${usage.video.maxDurationSeconds}s · ${usage.video.quality} · audio ${usage.video.audio ? 'on' : 'off'} · aspect ratio ${usage.video.aspectRatios.join(', ')}; aspect ratio does not change cost.`}>{usage.features.videoAd.durations.map((item) => <CostRow key={item.durationSeconds} label={`${item.durationSeconds} seconds`} cost={item.credits} available={usage.credits.available} />)}</FeatureCard></div>
