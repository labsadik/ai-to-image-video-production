...
import { AlertTriangle, CheckCircle2, Clock3, Download, Film, Loader2, ScanSearch, Trash2 } from 'lucide-react';
...
export function HistoryList({ initialItems }: { initialItems: HistoryItem[] }) {
  ...
  return <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
    {items.map(item => {
      ...
      const hasBothSizes = item.masterByteSize !== null && item.previewByteSize !== null;
      const sizeRatio = hasBothSizes && item.masterByteSize! > 0 ? `${((item.previewByteSize! / item.masterByteSize!) * 100).toFixed(1)}% of master` : null;
      const previewLabel = isVideo ? 'Preview poster' : 'Low-size preview';
      return <article key={item.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        <Link href={`/dashboard/media?history=${item.id}`} className="block">
          ...
          <div className="p-5">
            ...
            {hasBothSizes && <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Master</p><p className="mt-1 text-xs font-semibold text-slate-800">{bytesLabel(item.masterByteSize)}</p><p className="mt-0.5 text-[10px] text-slate-400">Full downloadable asset</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">{previewLabel}</p><p className="mt-1 text-xs font-semibold text-slate-800">{bytesLabel(item.previewByteSize)}</p><p className="mt-0.5 text-[10px] text-slate-400">Used for fast loading</p></div></div>}
            {sizeRatio && <p className="mt-2 text-[11px] text-slate-400">Preview size: {sizeRatio}.</p>}
          </div>
        </Link>
