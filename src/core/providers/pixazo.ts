...
export class PixazoImageAdapter implements ProviderAdapter {
  ...
  async generate(request: GenerationRequest & { model: string; apiKey: string }): Promise<ProviderResult> {
    ...
    const response = await withTimeout(fetch(endpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Ocp-Apim-Subscription-Key': request.apiKey },
      body: JSON.stringify({
    ...
    return { externalId: mediaUrl, mimeType, base64: buffer.toString('base64'), providerMetadata: { mediaUrl, model: request.model } };
  }

  async healthCheck(apiKey: string, model = 'flux') {
    const started = Date.now();
    try {
      const response = await withTimeout(fetch(`https://gateway.pixazo.ai/${encodeURIComponent(model)}/text-to-image`, { method: 'OPTIONS', headers: { 'Ocp-Apim-Subscription-Key': apiKey } }), 10_000);
