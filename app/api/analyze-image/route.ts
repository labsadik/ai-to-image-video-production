...
import { analyzeImageWithGroq } from '@/core/providers/groq';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const projectId = typeof rawProjectId === 'string' && rawProjectId.length ? rawProjectId : undefined;
    ...
    if (route.protocol !== 'groq_vision') throw new Error(`Image analysis route must use groq_vision; received ${route.protocol}`);
    ...
    const analysis = await analyzeImageWithGroq({ base64: storedBytes.toString('base64'), mimeType: stored.mimeType, level, apiKey, model: route.model });
    ...
    const analysisJobRequest = {
      ...
      width, height, previewWidth: preview.width ?? width, previewHeight: preview.height ?? height, storedMaxWidth: 1920,
      storedQuality: stored.quality, previewMaxWidth: 640, previewQuality: 60, latencyMs: Date.now() - started,
      safetyApplied: false, storageRole: 'compressed_master + compressed_preview', projectId: projectId ?? null,
    };

    const { data: job, error: jobError } = await admin.from('generation_jobs').insert({
      id, user_id: user.id, project_id: projectId ?? null, status: 'processing', operation: 'analyzeImage',
      prompt: `Image authenticity analysis · ${level}`, size: width && height ? `${width}x${height}` : `${stored.byteSize} bytes`,
      quality: 'preview', provider: analysis.provider, model: analysis.model, reserved_credits: credits, idempotency_key: idempotencyKey,
      started_at: new Date(started).toISOString(), request: analysisJobRequest,
    }).select('id').single();
    ...
    const { error: completeError } = await admin.from('generation_jobs').update({ status: 'succeeded', reserved_credits: 0, completed_at: new Date().toISOString(), request: analysisJobRequest }).eq('id', id).eq('user_id', user.id);
    ...
    return NextResponse.json({
      jobId: id, provider: analysis.provider, model: analysis.model, level, credits, imageSha256: sha256,
      safetyApplied: false, projectId: projectId ?? null, result: analysis.result,
    ...
    if (admin && jobId) {
      try { await admin.from('generation_outputs').delete().eq('job_id', jobId); } catch {}
      try { await admin.from('assets').delete().eq('user_id', userId).eq('metadata->>job_id', jobId); } catch {}
      try { await admin.from('generation_jobs').delete().eq('id', jobId).eq('user_id', userId); } catch {}
    }
    if (admin && reserved && !finalized) {
      try { await admin.rpc('refund_generation_credits', { p_user_id: userId, p_amount: credits, p_idempotency_key: `${idempotencyKey}:refund` }); } catch {}
    }
