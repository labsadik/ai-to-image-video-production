...
begin
  if p_amount <= 0 then raise exception 'invalid addon credit amount'; end if;
  ...
  values(p_user_id,'addon_grant',p_amount,(select credits from public.profiles where id=p_user_id),p_idempotency_key,coalesce(p_metadata,'{}'::jsonb));
  return true;
end;
$function$;
...
begin
  ...
  set plan=p_plan_id,plan_id=p_plan_id,monthly_credits=v_plan_credits,credits=greatest(credits+v_delta,0),credit_period_start=coalesce(p_period_start::date,current_date),credit_period_end=coalesce(p_period_end::date,(current_date+interval '1 month')::date),updated_at=now()
  where id=p_user_id;
  insert into public.credit_ledger(user_id,kind,amount,balance_after,idempotency_key,metadata)
  values(p_user_id,'monthly_grant',v_delta,(select credits from public.profiles where id=p_user_id),p_idempotency_key,jsonb_build_object('plan',p_plan_id,'period_start',p_period_start,'period_end',p_period_end) || coalesce(p_metadata,'{}'::jsonb));
  return true;
end;
$function$;
