...
begin
  for m in
    select *
    from pgmq.read(
      'solamentis_generation',
      greatest(p_visibility_seconds, 30),
      least(greatest(p_quantity, 1), 20)
    )
  loop
...
grant execute on function public.claim_generation_messages(integer, integer) to service_role;
