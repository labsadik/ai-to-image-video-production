...
function stripeSubscriptionStatus(value: unknown) { const raw = String(value ?? ""); if (["active", "trialing", "past_due", "unpaid", "canceled", "incomplete"].includes(raw)) return raw === "canceled" ? "cancelled" : raw; if (raw === "incomplete_expired") return "incomplete"; return "incomplete"; }
  ...
  try {
    ...
    if (eventType === "checkout.session.completed" || eventType === "checkout.session.async_payment_succeeded") {
      ...
      if (object.mode === "subscription" && userId && planId) {
        ...
        const initialStatus = object.payment_status === "paid" || object.payment_status === "no_payment_required" ? "active" : "incomplete";
        const { error: subError } = await admin.from("subscriptions").upsert({ user_id: userId, plan_id: planId, status: initialStatus, provider: "stripe", external_customer_id: object.customer ?? null, external_subscription_id: object.subscription ?? null, external_checkout_session_id: String(object.id), metadata: { country_code: object.metadata?.country_code ?? null, billing_period: billingPeriod } }, { onConflict: "user_id" });
